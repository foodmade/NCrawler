...
function doParse(htmlString){
    ...
    try{
        ...
        result.totalHits = resObj.Summary.ResultCount;
        ...
        for (var j= 0;j < resObj.data.adpos.data.length; ++j) {
            var adpo=resObj.data.adpos.data[j];
            adpos_items.push({
                skuId:adpo.sku_id,
                skuTitle:adpo.ad_title,
                flowOrder:adpo.flow_order
            });
        }
    ...
    result.items = items;
    result.adpos_items=adpos_items;
