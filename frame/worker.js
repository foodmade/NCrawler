...
var _WORKER = module.exports = {
    ...
    doCrawling:function(){
        ...
        var taskData = THIS_MODULE._CUR_TASK_INFO.task.taskData;
        var reqOptions = taskData.crawlingOptions;
        THIS_MODULE.log('Crawling options check, PATH:' + reqOptions.path, config.LOG._LOG_LEVEL_INFO);
        ...
        //THIS_MODULE.log('Do crawling http request', config.LOG._LOG_LEVEL_WARNING);
        //utils.httpRequest_GET(reqOptions,__OnHttpReqError, __onHttpRsp);

        taskData.timeStartCrawling = new Date();

        utils.httpRequest(reqOptions, __OnHttpReqError, __onHttpRsp);

        //////////////////////////////////////////////////////////////////////////////////////////////////////////////

        function __onHttpRsp(rsp, rspDataBuff){
            ...
            if(rsp.statusCode == '200'){
                ...
                THIS_MODULE.doCrawlingRspDataPreFormating(rsp.headers, rspDataBuff, function(err, unzippedDataBuff){

                    if(!err){
                        THIS_MODULE.doParsingEx(rsp.headers, unzippedDataBuff.toString(), unzippedDataBuff);
                    }else{
                        THIS_MODULE.log('Crawling rsp data pre formating failed, error:' + err.message + ', Task Over！',
                ...
                THIS_MODULE.log('Do crawling encounter HTTP ' + rsp.statusCode + ', data:' + rspDataBuff.toString(), config.LOG._LOG_LEVEL_WARNING);
                var location = rsp.headers['location'];
                THIS_MODULE.log('Do crawling encounter HTTP ' + rsp.statusCode + ', location:' + location, config.LOG._LOG_LEVEL_WARNING);
                ...
                //THIS_MODULE.log('__OnHttpReqError() call doTaskOver()', config.LOG._LOG_LEVEL_WARNING);
        ...
        function __OnHttpReqError(e){
            //THIS_MODULE.log('__onHttpRsp() call doTaskOver()', config.LOG._LOG_LEVEL_WARNING);
    ...
    doCrawlingRspDataPreFormating:function(rspHeaders, rspDataBuff, callback){
        var THIS_MODULE = this;
        var encoding = rspHeaders['content-encoding'];
        if(encoding && encoding != 'identity'){
            THIS_MODULE.log('Do unzip rsp data, encoding:' + encoding,
                config.LOG._LOG_LEVEL_WARNING);
            zlib.unzip(rspDataBuff, function(err, unzippedDataBuff) {
                callback(err, unzippedDataBuff);
            });
        }else{
            THIS_MODULE.log('Do unzip rsp data, not encoded:',
                config.LOG._LOG_LEVEL_WARNING);
            callback(null, rspDataBuff);
        }
    },
    doParsingEx:function(headers,htmlString, dataBuff){

        var THIS_MODULE = this;
        var result = dynamic_parser.doParse(htmlString, dataBuff);
    ...
    doCommitTaskResult:function(result, callbak){
        ...
        if(result){
           ...
           THIS_MODULE.log('Do commit task result:' + JSON.stringify(postData), config.LOG._LOG_LEVEL_WARNING);
    ...
    doCrawlingError:function(){
        var THIS_MODULE = this;
        //THIS_MODULE.log('doCrawlingError()', config.LOG._LOG_LEVEL_WARNING);
        THIS_MODULE.doTaskOver();
    },
    doTaskOver:function(){
        var THIS_MODULE = this;
...
process.on('message', function(msg){
    ...
    switch(msg.msg){
        case _PIPE_MSG_WORKER_RUN_REQ:
            //_WORKER.log('Worker run command!', config.LOG._LOG_LEVEL_ERROR);
            _WORKER.api_run(msg);
            break;
